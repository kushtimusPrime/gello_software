...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    #ur5e
    '/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT7WBMUB-if00-port0': DynamixelRobotConfig(
        joint_ids=(1,2,3,4,5,6),
        joint_offsets=(2*np.pi/2, 1*np.pi/2, 0*np.pi/2, -1*np.pi/2, 2*np.pi/2, -1*np.pi/2),
        joint_signs=(1, 1, -1, 1, 1, 1),
        gripper_config=(7,10.083203125,-31.716796875))
}
